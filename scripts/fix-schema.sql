-- Fix schema - create tables only (enums already exist)
-- Run this in Supabase SQL Editor

-- Enable UUID extension (safe to run if exists)
...
CREATE INDEX IF NOT EXISTS idx_users_foundation ON users(foundation_id);
CREATE INDEX IF NOT EXISTS idx_users_auth ON users(auth_id);
CREATE INDEX IF NOT EXISTS idx_organizations_foundation ON organizations(foundation_id);
CREATE INDEX IF NOT EXISTS idx_grants_foundation ON grants(foundation_id);
CREATE INDEX IF NOT EXISTS idx_grants_organization ON grants(organization_id);
CREATE INDEX IF NOT EXISTS idx_grants_status ON grants(status);
CREATE INDEX IF NOT EXISTS idx_grant_reviews_grant ON grant_reviews(grant_id);
CREATE INDEX IF NOT EXISTS idx_grant_comments_grant ON grant_comments(grant_id);
CREATE INDEX IF NOT EXISTS idx_meetings_foundation ON meetings(foundation_id);
CREATE INDEX IF NOT EXISTS idx_meetings_datetime ON meetings(date_time);
CREATE INDEX IF NOT EXISTS idx_documents_entity ON documents(entity_type, entity_id);
CREATE INDEX IF NOT EXISTS idx_invitations_token ON invitations(token);
CREATE INDEX IF NOT EXISTS idx_activity_foundation ON activity_log(foundation_id);
...
DROP TRIGGER IF EXISTS update_foundations_updated_at ON foundations;
DROP TRIGGER IF EXISTS update_users_updated_at ON users;
DROP TRIGGER IF EXISTS update_organizations_updated_at ON organizations;
DROP TRIGGER IF EXISTS update_grants_updated_at ON grants;
DROP TRIGGER IF EXISTS update_grant_reviews_updated_at ON grant_reviews;
DROP TRIGGER IF EXISTS update_grant_comments_updated_at ON grant_comments;
...
DROP POLICY IF EXISTS "Users can view their foundation" ON foundations;
DROP POLICY IF EXISTS "Primary advisors can update their foundation" ON foundations;
DROP POLICY IF EXISTS "Users can view users in their foundation" ON users;
DROP POLICY IF EXISTS "Primary advisors can manage users" ON users;
DROP POLICY IF EXISTS "Users can update their own profile" ON users;
DROP POLICY IF EXISTS "Users can view orgs in their foundation" ON organizations;
DROP POLICY IF EXISTS "Advisors+ can create orgs" ON organizations;
DROP POLICY IF EXISTS "Advisors+ can update orgs" ON organizations;
DROP POLICY IF EXISTS "Primary advisors can delete orgs" ON organizations;
DROP POLICY IF EXISTS "Users can view grants in their foundation" ON grants;
DROP POLICY IF EXISTS "Advisors+ can create grants" ON grants;
DROP POLICY IF EXISTS "Advisors+ can update grants" ON grants;
DROP POLICY IF EXISTS "Primary advisors can delete grants" ON grants;
DROP POLICY IF EXISTS "Users can view reviews in their foundation" ON grant_reviews;
DROP POLICY IF EXISTS "Advisors can create reviews" ON grant_reviews;
DROP POLICY IF EXISTS "Reviewers can update their own reviews" ON grant_reviews;
DROP POLICY IF EXISTS "Users can view comments" ON grant_comments;
DROP POLICY IF EXISTS "Users can create comments" ON grant_comments;
DROP POLICY IF EXISTS "Users can update their own comments" ON grant_comments;
DROP POLICY IF EXISTS "Users can view meetings" ON meetings;
DROP POLICY IF EXISTS "Advisors can manage meetings" ON meetings;
DROP POLICY IF EXISTS "Users can view documents" ON documents;
DROP POLICY IF EXISTS "Advisors+ can upload documents" ON documents;
DROP POLICY IF EXISTS "Advisors can delete documents" ON documents;
DROP POLICY IF EXISTS "Primary advisors can manage invitations" ON invitations;
DROP POLICY IF EXISTS "Anyone can view invitation by token" ON invitations;
DROP POLICY IF EXISTS "Users can view activity in their foundation" ON activity_log;
...
-- Storage bucket for documents (ignore error if exists)
